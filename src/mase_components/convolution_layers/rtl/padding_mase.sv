...
//     parameter DATA_IN_0_PARALLELISM_DIM_0 = 2,  // parallelism for width
//     parameter DATA_IN_0_PARALLELISM_DIM_1 = 2,  // parallelism for height
//     parameter DATA_IN_0_PARALLELISM_DIM_3 = 1,  // parallelism for batch
//     parameter DATA_OUT_DIM0 = 2,
//     parameter DATA_OUT_DIM1 = 2,  // I define this same for no matter whatever parallelism
//     parameter DATA_OUT_0_PARALLELISM_DIM_3 = 1

// ) (
//     input clk,
//     input rst,
//     input  logic [DATA_IN_PRECISION_0-1:0] data_in_array [DATA_IN_0_PARALLELISM_DIM_0*DATA_IN_0_PARALLELISM_DIM_1*DATA_IN_0_PARALLELISM_DIM_3-1:0],
//     input logic [DATA_IN_PRECISION_0-1:0] data_in,
//     input logic data_in_valid,
//     output logic data_in_ready,

//     output logic [DATA_IN_PRECISION_0-1:0] data_out_array [DATA_OUT_DIM0*DATA_OUT_DIM1*DATA_OUT_0_PARALLELISM_DIM_3],
//     output logic [DATA_IN_PRECISION_0-1:0] data_out,
//     output logic data_out_valid,
//     input logic data_out_ready
// );
//   //for input array, e.g. if (15,24,9,20), in[0] is 20, in[1] is 9 etc.
//   //Hence if 2 batches, in[3...7] are for batch one

//   // the step increase of counter will depend on parallelism in x and y direction
//   localparam stepsize_x = DATA_OUT_DIM0;
//   localparam stepsize_y = DATA_OUT_DIM1;

//   // for the input data, number of input for each batch entered depends on Parallel_dim_0 and Parallel_dim_1
//   // The first DATA_IN_0_PARALLELISM_DIM_0 * DATA_IN_0_PARALLELISM_DIM_1 is for batch 1
//   // The second DATA_IN_0_PARALLELISM_DIM_0 * DATA_IN_0_PARALLELISM_DIM_1 is for batch 2
//   localparam num_ele_in_each_batch = DATA_IN_0_PARALLELISM_DIM_0 * DATA_IN_0_PARALLELISM_DIM_1;
...
//   // since for all batches, they are same dimensions, we can use same x,y counter to track
...
//   // Internal storage for the entire array of pixels
//   // Each element is DATA_IN_PRECISION_0 bits wide
//   logic [DATA_IN_PRECISION_0-1:0] stored_pixel;
//   logic [DATA_IN_PRECISION_0-1:0] stored_pixel_array [DATA_IN_0_PARALLELISM_DIM_0*DATA_IN_0_PARALLELISM_DIM_1*DATA_IN_0_PARALLELISM_DIM_3-1:0];


//   // 2D buffer: 8×8: to store two rows
//   // logic [PIXEL_WIDTH-1:0] line_buffer[7:0][7:0];

//   // (buffer_x, buffer_y) track where we write in the buffer.
//   // We increment x by 2 each time, and every time x wraps around from 6 -> 0,
//   // we increment y by 2.
//   logic [2:0] buffer_x;  // can go up to 7
//   logic [2:0] buffer_y;  // can go up to 7

//   localparam int CHUNK_X = DATA_IN_0_PARALLELISM_DIM_0;
//   localparam int CHUNK_Y = DATA_IN_0_PARALLELISM_DIM_1;
//   // Define 2D buffer lines based on how many parallelism I have:
//   // If I have parallelism of 2x2,
//   // If I have parallelism of 4x4, my buffer is
//   // Allows you to store CHUNK_X*CHUNK_Y in one cycle
//   // chunk_buffer[0][0] gets data_in[0]
//   // chunk_buffer[0][1] gets data_in[1]
//   // chunk_buffer[1][0] gets data_in[2]
//   // chunk_buffer[1][1] gets data_in[3]
//   // always_ff @(posedge clk or posedge rst) begin
//   //   if (rst) begin
//   //     // optional reset of buffer
//   //     for (int y = 0; y < CHUNK_Y; y++) begin
//   //       for (int x = 0; x < CHUNK_X; x++) begin
//   //         chunk_buffer[x][y] <= '0;
//   //       end
//   //     end
...
//   //     for (int y = 0; y < CHUNK_Y; y++) begin
//   //       for (int x = 0; x < CHUNK_X; x++) begin
//   //         chunk_buffer[x][y] <= data_in[y*CHUNK_Y+x];
//   //       end
//   //     end
//   //   end
//   // end
...
//     // Initial state in in IDLE state
...
//             // We got our array of input => store it
