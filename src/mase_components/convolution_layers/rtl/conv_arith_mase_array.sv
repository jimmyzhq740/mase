...
module conv_arith_mase_array #(
  ...
  //   if (rst) begin
  //     channel_counter <= 0;
  //   end else if (channel_counter + DATA_OUT_0_PARALLELISM_DIM_2 >= WEIGHT_TENSOR_SIZE_DIM_3) begin
  //     channel_counter <= channel_counter;
  //     // when image_done == 1, it means one set out channels have finished, go to next set
  //   end else if (image_done == 1) begin
  //     channel_counter <= channel_counter + DATA_OUT_0_PARALLELISM_DIM_2; // goes to find 3rd channel
  //   end
  //   // when striding_input_buffer is valid, selected data selects from buffer
  //   if (channel_counter >= DATA_OUT_0_PARALLELISM_DIM_2) begin
  //     int i
  //     for (i=0; i<DATA_IN_0_TENSOR_SIZE_DIM_0*DATA_IN_0_TENSOR_SIZE_DIM_1; i++) begin
  //       data_from_striding_buffer
  //     end
  //     if (striding_input_buffer_out_valid) begin
  //       assign selected_data_in = data_from_striding_buffer;
  //       assign selected_data_in_valid = 'b1;
  //     end
  //   end else if (data_in_valid) begin
  //     assign selected_data_in = data_in;
  //     selected_data_in_valid = 'b1;
  //   end else begin
  //     selected_data_in_valid = 'b0;
  //   end
  // end
  ...
  always_ff @(posedge clk) begin
    ...
    case (state)
      IDLE: begin
        selected_data_in_valid <= 'b0;  // IDLE state: dont let arith to start
        ...
        if (channel_counter>= DATA_OUT_0_PARALLELISM_DIM_2&&striding_input_buffer_out_valid) begin
          ...
          //     data_from_striding_buffer[TOTAL_STRIDING-i] = buffer_array_mul[striding_data_counter][(TOTAL_STRIDING-i)*DATA_IN_0_PRECISION_0-1 -: DATA_IN_0_PRECISION_0];
          //   end
          //   selected_data_in_valid <= 'b1;
          //   selected_data_in <= data_from_striding_buffer;
  ...
  generate
    for (i = 0; i < DATA_OUT_0_PARALLELISM_DIM_2; i++) begin : parallel_blocks
      conv_arith_mase #(
          ...
          .arith_data_out(debug_arith_data_out[i]),
